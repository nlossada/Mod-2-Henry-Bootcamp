...
//VER SI FALTA INSTANCIAR LA CLASE TODO para pasar como argumento.. entiendo que se pasa como arg el objeto ya instanciado
...
function buildToDos(toDos) {
  // Tu código acá:
  const toDosArr = toDos.map(buildToDo);
  return toDosArr
}
...
function displayToDos() {
  ...
  const arrBuildToDos = buildToDos(toDoItems);
  for (let i = 0; i < arrBuildToDos.length; i++) {
    toDoContainer.appendChild(arrBuildToDos[i]);
  }
}
...
function completeToDo(event) {
  ...
  // Tu código acá:
  toDoItems[index].completeToDo();        //ESTE NO ME PASA EL TEST
  displayToDos();
}

//ARMAR BOTON DE RESET DE TODA LA LISTA PARA LIMPIAR
